...
for year in range(1994, 2018):
    ...
    where_clause_ff = "Merged_Postfire_Ig_Year = {}".format(year)
    arcpy.SelectLayerByAttribute_management("Overlap_Feature_Joined", "NEW_SELECTION", where_clause_ff)

    # Use Tabulate Area tool with OVERLAP_OID as feature zone data
    input_zone = "Overlap_Feature_Joined"
    zone_field = "Overlap_Table_OVERLAP_OID"
    input_raster = "mtbs_CA_{}.tif".format(year)
    class_field = "Value"
    output_tabulate = "tabulate_merged_{}".format(year)
    output_zonal = "zonal_merged_{}".format(year)
...
tableList = [table for table in arcpy.ListTables() if "tabulate_merged" in table]
...
tableList = [table for table in arcpy.ListTables() if "zonal_merged" in table]
...
join_field_tabulate = "OVERLAP_TABLE_OVERLAP_OID"
